# 主審之眼 — 接手說明

## 跟使用者溝通

用白話文，不要丟程式術語和檔名清單。講「負責畫 3D 的繪圖工具」而不是「three.js r128」，
講「放進正式版」而不是「merge 到 main」。使用者要知道的是發生什麼事、影響是什麼，
不是實作細節。

## Artifact

這個遊戲一直是從這個 repo 的 HTML 發布到同一個 Artifact：

    https://claude.ai/artifact/AQhdAFmQqFZsTUfjHXYJj2

**要更新一定要用 `url` 參數指向它**，否則會變成一個全新的 artifact、連結就換掉了。
檔案已經從 `umpire-eye.html` 改名成 `index.html`，直接發布必然產生新的 artifact。

發布時 `vendor/` 和 `*-model.js` 都要一起當作附帶檔案送上去，否則沙箱裡載不到。

## 不要動的東西

這三項都是使用者花時間量測、微調定案的，不是隨手寫的預設值。

**1. `game.js` 的 `BROADCAST_DEFAULT`（主審視角）**
照使用者提供的 CPBL 主審面罩攝影機照片推算出來：量了壘線斜率和本壘板的透視壓縮，
再由使用者自己微調定案。**看起來「太廣角」是對的，不要修。**

**2. `rig_common.py` 的骨頭長度**
`THIGH`、`SHIN`、`UPPER_ARM`、`FOREARM`、`SHOULDER`、`STAND_HIP` 都不要改。
投球動作的關鍵格是拿手腳的世界座標當目標點，改了骨長動作就會跑掉
（手構不到目標會整支打直）。**要調外型只改 `_body_profile` 的半徑。**

**3. 人物比例與配色**
照 `reference/player_front.jpg` 做的：頭身比 7.2、頭寬 0.24 公尺、藍衣白褲白內搭、
深藍襪加兩道白條。比例是從同一角色的 3D 重建量橫切面得到的。

## 地雷

**有貼圖的模型在 Artifact 沙箱裡會整個載入失敗。**
GLTFLoader 用 `ImageBitmapLoader`，內部走 `fetch`，而沙箱的 CSP 不允許 fetch
`blob:` 或 `data:`。兩道對策缺一不可：`blender/glb_datauri.py` 把貼圖改寫成 `data:` URI，
加上 `scene.js` 開頭幫 `data:image/` 註冊 `TextureLoader`（走 `<img>`）。
用 `python blender/devserver.py . --csp` 可以在本機重現沙箱的 CSP 來驗證。

**`blender/build_pitcher_tripo.py` 目前沒用。**
那是接外部模型的流程。那個模型的手臂比骨架短 25%，轉移後出手高度從 1.58 掉到
1.43 公尺，動作會怪。要接外部模型請優先找本身就帶投球動畫的。

**本機測試時瀏覽器會快取 `.js`。**
`devserver.py` 已經送 `no-store`，但換過埠號比較保險。

## 玩法已經定案，不要「補回來」

- 不顯示好壞球數、出局數
- 沒有 ABS 挑戰
- 判決只有三種字樣：正確／誤判／超時
- 打者每 5 球換一位（身高變，好球帶跟著變），一場 15 球剛好三位
- 得分依難度浮動，看的是球離好球帶邊緣多遠（`margin` 絕對值），好球壞球一視同仁：
  貼邊最難，判對最多分、判錯罰最輕；深入或差很遠的一眼可辨，判對少給、判錯重罰。
  用公尺原值算到小數點以下，最後才四捨五入成整數。**不要改回固定分數。**
- 判決速度加分也是連續的，不是門檻式的：從球通過本壘板算起，0.4 秒內滿分 +30，
  到 1.6 秒線性遞減到 0，秒數差 0.1 秒就看得出分數差別。只有判對才加。
- 音效用使用者自己的音檔（`audio/`，MP3）。**這是使用者指定的，先前「一律用
  合成音」那條已經作廢。** 合成音留著當備援：檔案載不到就自動退回去，遊戲不會沒聲音。
  - `ball` 按下壞球、`strike` 按下好球、`catch` 進手套、`happy` 判對、`boo` 誤判、
    `fans` 球場背景（開打淡入、結算淡出、循環）
  - 出手風切、超時、結算這三個使用者沒給檔案，仍是合成音
  - 檔名一律小寫。網站伺服器區分大小寫，`strike.MP3` 會抓不到
  - 新音檔進來一定要先跑 `python tools/strip_audio_tags.py audio`：下載來的音效常常
    內嵌幾百 KB 的封面圖，這批原本 4.9 MB，剝掉標籤後只剩 0.64 MB，音訊一個位元組沒動
  - 判決音（happy／boo）要等按鍵音（strike／ball）發完才響，不能疊在一起。
    `play(name, mode)`：按鍵音用 `'start'`（立刻響並重開排隊），判決音用 `'queue'`。
    **按鍵音一定要 `'start'`**——它是按鈕回饋，被延後就像按鈕卡住；而且 happy 長 4 秒，
    不重開排隊的話下一球判得快就會被前一球的判決音卡到。
  - 排隊等的是「實際有聲音的長度」（`audibleLength`），不是整個檔案長度。
    ball 全長 1 秒但 0.77 秒後就沒聲音了，死等整檔會多出 0.23 秒空檔。
  - 背景人聲的音量（`CROWD_LEVEL`）要明顯低於判決音，不然會蓋掉
  - 循環點各往內縮 50 毫秒：MP3 解碼前後會多出靜音，不縮會有「喀」一聲
  - 右上角有開關，選擇記在瀏覽器裡。iOS 要有使用者動作才能出聲，所以第一次
    點擊或按鍵時才建立音訊環境、也才開始下載音檔。
- 球通過本壘板到進手套之間有約 50 毫秒的空窗，判得夠快會卡在這裡。`finishPitch`
  會在那時補一聲手套聲，不然音效會時有時無，聽起來像壞掉。
- 得分顯示在判決文字右邊
- 重播的距離標籤要離球 46px、上方 34px，不能擋到球
- 場地尺寸全照規則書，界外線從打擊區前緣才開始畫（不穿過打擊區和本壘板）

## 離線快取

`sw.js` 會預先快取遊戲需要的全部檔案，所以 three.js 放在 `vendor/` 而不是走 CDN。

> **改過任何遊戲檔案之後，一定要把 `sw.js` 裡的 `VERSION` 加一。**
> 快取整批換新是靠這個，忘了加使用者就不會拿到新版。

網頁本身跟其他檔案走同一套 cache-first 規則。**不要把 `index.html` 改成 network-first**——
曾經那樣做過，結果是新的說明文字配上舊的遊戲程式（說明寫 9 球、玩起來 15 球）。
要嘛全舊要嘛全新，中間狀態不能存在。

更新是這樣運作的：`index.html` 用 `updateViaCache: "none"` 註冊，並在每次開啟時主動
呼叫一次 `update()`（瀏覽器自己的檢查會偷懶，從主畫面啟動的 App 尤其明顯）。新版接手時
`controllerchange` 會觸發頁面自動重新載入一次，使用者只要打開一次就全部換新。
第一次安裝沒有舊版可換，那時不重載。
